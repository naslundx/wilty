...
def create_game(req: CreateGameReq):
    ...
    cursor.execute("INSERT INTO users (id, game_id, username, is_creator) VALUES (?, ?, ?, 1)", (user_id, game_id, req.username))
    engine.commit()
...
def join_game(req: JoinGameReq):
    ...
    user_id = str(uuid.uuid4())
    cursor.execute("INSERT INTO users (id, game_id, username) VALUES (?, ?, ?)", (user_id, target_code, req.username))
    engine.commit()
...
def add_statement(req: StatementReq):
    cursor = engine.get_cursor()
    cursor.execute("INSERT INTO statements (game_id, user_id, text, category) VALUES (?, ?, ?, 'User')", (req.game_id, req.user_id, req.text))
    engine.commit()
...
def start_game(req: StartGameReq):
    ...
    if not user or not user['is_creator']:
        raise HTTPException(status_code=403, detail="Only the room host can start the match.")
    ...
    cursor.execute("SELECT COUNT(*) as count FROM statements WHERE game_id = ?", (req.game_id,))
    if cursor.fetchone()['count'] == 0:
        raise HTTPException(status_code=400, detail="Statement pool is empty. Please check a category.")
...
def get_state(game_id: str, user_id: str):
    ...
    cursor.execute("SELECT id, username, score, is_creator FROM users WHERE game_id = ?", (game_id,))
    players = [dict(row) for row in cursor.fetchall()]

    if not any(p['id'] == user_id for p in players):
        raise HTTPException(status_code=404, detail="User session not found")

    cursor.execute("SELECT id, text FROM statements WHERE id = ?", (game['current_statement_id'],))
    st = cursor.fetchone()
    ...
    cursor.execute("SELECT vote FROM votes WHERE game_id = ? AND user_id = ?", (game_id, user_id))
    ...
    cursor.execute("SELECT rating FROM prompt_ratings WHERE statement_id = ? AND user_id = ?", (statement_id, user_id))
...
def post_vote(req: VoteReq):
    cursor = engine.get_cursor()
    cursor.execute("INSERT OR REPLACE INTO votes (game_id, user_id, vote) VALUES (?, ?, ?)", (req.game_id, req.user_id, req.vote))
    engine.commit()
    return {"status": "voted"}
...
def rate_prompt(req: RatePromptReq):
    ...
    cursor = engine.get_cursor()
    try:
        cursor.execute("INSERT INTO prompt_ratings (statement_id, user_id, rating) VALUES (?, ?, ?)",
                       (req.statement_id, req.user_id, req.rating))
        engine.commit()
    except sqlite3.IntegrityError:
        raise HTTPException(status_code=400, detail="You have already rated this statement prompt.")
    return {"status": "rated"}

@app.post("/api/game/resolve")
def resolve_round(req: ResolveReq):
    cursor = engine.get_cursor()
    cursor.execute("SELECT current_storyteller_id, current_statement_id, round_started_at FROM games WHERE id = ?", (req.game_id,))
    game = cursor.fetchone()
    ...
    cursor.execute("SELECT COUNT(*) as count FROM votes WHERE game_id = ?", (req.game_id,))
    total_votes = cursor.fetchone()['count']
    ...
    if total_votes < (total_players - 1) and elapsed < config.ROUND_TIMEOUT_LIMIT:
        raise HTTPException(status_code=400, detail="Cannot resolve yet. Waiting for other responses or timeout.")
    ...
    for vote in cursor.fetchall():
        if vote['vote'] == req.actual_truth:
            cursor.execute("UPDATE users SET score = score + 1 WHERE id = ?", (vote['user_id'],))

    cursor.execute("UPDATE statements SET used = 1 WHERE id = ?", (game['current_statement_id'],))
    cursor.execute("SELECT MAX(score) as max_score FROM users WHERE game_id = ?", (req.game_id,))
